		...
		<section class="p-4">
			{#if isLoading}
				<Accordion>
					...
					<AccordionItem>
						<svelte:fragment slot="summary"
							><div class="placeholder animate-pulse" /></svelte:fragment
						>
					</AccordionItem>
					<!-- ... -->
				</Accordion>
						...
						<button type="button" class="btn btn-sm variant-ringed-secondary">
							...
							<a
								...
								on:click={async (e) => {
									...
									const href = `/pokemon/${pokemon.prevEvolution.id}`;
									const result = await preloadData(href);

									if (result.type === 'loaded' && result.status === 200) {
										replaceState(href, { pokemon: result.data });
									} else {
								...
								<button type="button" class="btn btn-sm variant-ringed-secondary">
									<a
										...
										on:click={async (e) => {
											...
											const href = `/pokemon/${evolution.id}`;
											const result = await preloadData(href);
